...
@RestController
@RequestMapping("/denuncias-anonimas")
public class DenunciaAnonimaController {
   ...
   @PostMapping("/save")
    public ResponseEntity<?> save(@RequestBody DenunciaAnonima denuncia) {
        denunciaService.salvar(denuncia);
         return ResponseEntity.status(HttpStatus.CREATED).body(denuncia);
}

 @GetMapping("/{id}")
    public ResponseEntity<Denuncia> getDenunciaAnonimaById(@PathVariable Long id) {
    ...
    @PutMapping("/{id}")
public ResponseEntity<Denuncia> atualizarDenunciaAnonima(
        @PathVariable Long id,
        @RequestBody DenunciaAnonima denunciaAtualizada) {
...
@DeleteMapping("/{id}")
public ResponseEntity<Void> deletarDenunciaAnonima(@PathVariable Long id) {
    if (denunciaService.buscarPorId(id) == null) {
        return ResponseEntity.notFound().build();
    }
